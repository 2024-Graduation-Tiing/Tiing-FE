...
export default function Filter() {
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const clickedCategory = e.currentTarget.parentElement?.firstChild?.textContent;
    ...
    if (clickedItem && clickedCategory) {
      if (searchParams?.getAll(clickedCategory).includes(clickedItem)) {
        ...
        const updatedValues = currentValues.filter((value) => value !== clickedItem);
      ...
      {Object.entries(FILTERS).map(([key, value]) => (
        <div className="mb-3 flex items-center" key={key}>
          ...
          {value.options.map((option) => (
            <button className="select-btn-default mr-4 border-0" key={option.id} onClick={handleClick}>
              {option.name}
            </button>
          ))}
        </div>
